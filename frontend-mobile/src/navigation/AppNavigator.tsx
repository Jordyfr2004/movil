...
function classifyProfileLoadError(
  ...
  if (status === 404) {
    return {
      kind: "not-found",
      message:
        `No pudimos encontrar tu perfil. ${message}`,
    };
  }
  ...
  if (
    ...
    return {
      kind: "network",
      message:
        `No pudimos comunicarnos con el servidor. ${message}`,
    };
  }
    ...
    <>
      {isSuperAdmin ? (
        ...
        <Stack.Navigator
          key={
            isAuthenticated
              ? `authenticated-${initialRouteName}`
              : "guest-stack"
          }
          initialRouteName={
            initialRouteName
